...
impl rlp::Decodable for UnverifiedTransactionWrapper {
	fn decode(d: &Rlp) -> Result<Self, DecoderError> {
		if is_typed_transaction(d) {
			...
			match d.as_raw()[0] {
				EIP2930_TX_TYPE => Ok(UnverifiedTransactionWrapper::Eip2930(UnverifiedEip2930Transaction::decode(d)?)),
				EIP1559_TX_TYPE => Ok(UnverifiedTransactionWrapper::Eip1559(UnverifiedEip1559Transaction::decode(d)?)),
...
impl UnverifiedTransactionWrapper {
	fn new(tx: TransactionWrapper, r: U256, s: U256, v: u64, chain_id: Option<u64>, hash: H256) -> Self {
		match tx {
			...
			TransactionWrapper::Eip2930(unsigned) => UnverifiedTransactionWrapper::Eip2930(UnverifiedEip2930Transaction {
				unsigned,
				r,
				s,
				v: v.try_into().unwrap(),
			...
			TransactionWrapper::Eip1559(unsigned) => UnverifiedTransactionWrapper::Eip1559(UnverifiedEip1559Transaction {
				unsigned,
				r,
				s,
				v: v.try_into().unwrap(),
	...
	fn rlp_append_sealed_transaction(&self, s: &mut RlpStream) {
		match self {
			UnverifiedTransactionWrapper::Legacy(tx) => {
				tx.rlp_append_sealed_transaction(s)
			},
			UnverifiedTransactionWrapper::Eip2930(tx) => {
				tx.rlp_append_sealed_transaction(s)
			},
	...
	fn validate_empty_sig(&self) -> bool {
		match self {
			...
			UnverifiedTransactionWrapper::Eip2930(_tx) => true, // TODO: what is it for eip2930?
...
mod tests {
	...
	fn legacy_sender_test() {
		...
		let t: UnverifiedTransactionWrapper = rlp::decode(&bytes).expect("decoding UnverifiedTransaction failed");
