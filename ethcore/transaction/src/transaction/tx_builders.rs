...
use super::{
	AccessList, Action, Bytes, Eip1559Transaction, Eip2930Transaction, LegacyTransaction,
	TransactionWrapper, TxType, U256,
};
use std::fmt;
...
impl TransactionWrapperBuilder {
	...
	pub fn with_priority_fee_per_gas(
		mut self,
	...
	pub fn build(self) -> Result<TransactionWrapper, TxBuilderError> {
		match self.tx_type {
			TxType::Legacy => Ok(TransactionWrapper::Legacy(LegacyTransaction {
				...
				gas_price: self
					.gas_price
					.ok_or_else(|| TxBuilderError::NoGasPriceSet)?,
				gas: self.gas,
				action: self.action,
				value: self.value,
				data: self.data,
